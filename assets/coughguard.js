// coughguard.js (triage + consent + send) – Patient‑friendly cough recorder for plain HTML sites
// Drop this file in /assets and link it with: <script src="assets/coughguard.js"></script>
// Optional: add the hidden Netlify form shown below to enable direct sending to your inbox.
...
(function(){
  ...
  host.innerHTML = `
    ...
    <div class="cg-wrap">
      ...
      <p class="cg-sub">Record your cough, add symptoms, and (with consent) send securely to your physio.</p>

      <div class="cg-card">
        <label class="cg-consent">
          <input id="cg-consent" type="checkbox" />
          <span>I understand this tool does not provide medical advice. I consent to share my recording and symptom details with my physiotherapist.</span>
        </label>
      ...
      <div class="cg-card">
        ...
        <div class="cg-col">
          ...
          <div>
            <label class="cg-label" for="cg-temp">Temperature °C (optional)</label>
            <input id="cg-temp" class="cg-input" type="number" step="0.1" inputmode="decimal" placeholder="e.g., 37.8" />
          </div>
          <div>
            <label class="cg-label">Breathlessness (0 none – 10 worst)</label>
            <input id="cg-breath" class="cg-input" type="range" min="0" max="10" step="1" value="0" />
          </div>
          <div>
            <label class="cg-label">Chest tightness (0 none – 10 worst)</label>
            <input id="cg-tight" class="cg-input" type="range" min="0" max="10" step="1" value="0" />
          </div>
          <div>
        ...
        <div class="cg-chiprow">
          ${[
            ['cg-fever','Fever/feverish'],
            ['cg-wheeze','Wheeze'],
            ['cg-chestpain','Chest pain'],
            ['cg-blue','Blue lips/face'],
            ['cg-confused','Confusion/drowsy'],
            ['cg-dehydr','Poor intake/low urine'],
            ['cg-runny','Runny/blocked nose'],
            ['cg-sorethroat','Sore throat']
          ].map(([id,label])=>`<label class="cg-chip"><input id="${id}" type="checkbox"/> ${label}</label>`).join('')}
        </div>
        ...
        <div class="cg-chiprow">
          ${[
            ['cg-asthma','Asthma'],
            ['cg-copd','COPD'],
            ['cg-bronch','Bronchiectasis'],
            ['cg-cf','Cystic fibrosis'],
            ['cg-heart','Heart condition'],
            ['cg-immuno','Immunosuppressed']
          ].map(([id,label])=>`<label class="cg-chip"><input id="${id}" type="checkbox"/> ${label}</label>`).join('')}
        </div>
      ...
      <div class="cg-card">
        <div class="cg-row">
          <button id="cg-send" class="cg-btn mic" disabled>Send to my physio</button>
          <button id="cg-export" class="cg-btn secondary">Download summary (.json)</button>
        </div>
        <div class="cg-footer">Tip: If sending fails, download the recording & summary and email them to your physio.</div>
      </div>

      <div class="cg-footer">Privacy: Your data stays in your browser unless you send or download it. Not a diagnostic tool. If you feel very unwell (severe breathlessness, blue lips, confusion), seek urgent care.</div>
    </div>
  `;
  ...
  const els = {
    consent: $('#cg-consent'),
    mic: $('#cg-mic'), start: $('#cg-start'), stop: $('#cg-stop'),
    audio: $('#cg-audio'), wave: $('#cg-wave'), msg: $('#cg-msg'),
    download: $('#cg-download'), send: $('#cg-send'), export: $('#cg-export'),
    days: $('#cg-days'), temp: $('#cg-temp'), breath: $('#cg-breath'), tight: $('#cg-tight'),
  ...
  els.consent.addEventListener('change', () => {
    const on = els.consent.checked;
    els.mic.disabled = !on; els.send.disabled = !on; els.start.disabled = !on; // start remains disabled until mic
  });

  // Input guards
  els.days.addEventListener('input', () => { const v = parseInt(els.days.value||''); if(isNaN(v)||v<0) els.days.value=''; });
  els.temp.addEventListener('input', () => { const v = parseFloat(els.temp.value||''); if(isNaN(v)||v<30||v>45) els.temp.value=''; });
  ...
  function startWave(){
    ...
    audioCtx = new AC(); const source = audioCtx.createMediaStreamSource(stream);
    analyser = audioCtx.createAnalyser(); analyser.fftSize = 1024; source.connect(analyser);
    const cvs = els.wave, ctx = cvs.getContext('2d'); const data = new Uint8Array(analyser.fftSize);
    function draw(){ analyser.getByteTimeDomainData(data); ctx.clearRect(0,0,cvs.width,cvs.height);
      ctx.lineWidth=2; ctx.strokeStyle='#111827'; ctx.beginPath(); const step=cvs.width/data.length; let x=0;
      for(let i=0;i<data.length;i++){ const y=(data[i]/128.0)*cvs.height/2; i?ctx.lineTo(x,y):ctx.moveTo(x,y); x+=step; }
      ctx.lineTo(cvs.width,cvs.height/2); ctx.stroke(); anim=requestAnimationFrame(draw); }
    cvs.width=cvs.clientWidth*(window.devicePixelRatio||1); cvs.height=cvs.clientHeight*(window.devicePixelRatio||1); draw();
  }
  ...
  els.mic.addEventListener('click', async () => {
    try{
      if(!navigator.mediaDevices?.getUserMedia){ setMsg('Recording is not supported in this browser. Try Chrome, Edge, or Safari.', false); return; }
      stream = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation:true, noiseSuppression:true } });
      els.start.disabled = false; els.mic.disabled = true; setMsg('Microphone enabled. Tap Start to record.', true); startWave();
    }catch(err){ setMsg('Microphone blocked. Allow mic access and reload.', false); }
  });
  ...
  els.start.addEventListener('click', () => {
    ...
    try{
      chunks = []; const rec = new MediaRecorder(stream, { mimeType: preferredMime() }); recorder = rec;
      rec.ondataavailable = e => { if(e.data && e.data.size>0) chunks.push(e.data); };
      rec.onstop = () => { const blob = new Blob(chunks, { type: rec.mimeType || 'audio/webm' }); latestBlob = blob;
        const url = URL.createObjectURL(blob); els.audio.src = url; els.audio.style.display='block'; els.download.disabled=false;
        setMsg('Recording saved. You can play it back below or send it to your physio.', true);
      };
      rec.start(); els.start.disabled=true; els.stop.disabled=false; setMsg('Recording… cough once or twice, then press Stop.', true);
  ...
  els.download.addEventListener('click', () => {
    if(!latestBlob) return; const a=document.createElement('a'); const url=URL.createObjectURL(latestBlob);
    a.href=url; a.download='coughguard_recording.webm'; document.body.appendChild(a); a.click(); setTimeout(()=>{document.body.removeChild(a); URL.revokeObjectURL(url);},0);
  });

  // Export summary JSON
  els.export.addEventListener('click', () => { const data = buildPayload(false); const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
    const a=document.createElement('a'); const url=URL.createObjectURL(blob); a.href=url; a.download='coughguard_summary.json'; document.body.appendChild(a); a.click(); setTimeout(()=>{document.body.removeChild(a); URL.revokeObjectURL(url);},0); });

  // Send to physio (tries Netlify form named "coughguard", else mailto guidance)
  els.send.addEventListener('click', async () => {
    ...
    if(form){
      ...
      // Fallback: open email draft with instructions
      const subject = encodeURIComponent('Cough Guard submission');
      const body = encodeURIComponent('Hi,\n\nI used Cough Guard to record my cough and filled in a symptom summary.\nI will attach the files I downloaded.\n\nThanks.');
      window.location.href = `mailto:?subject=${subject}&body=${body}`;
      setMsg('Email window opened. Please attach the downloaded recording and summary, then send.', true);
    }
  });
  ...
  function buildPayload(includePrivate){
    ...
    return {
      ...
      context: {
        daysUnwell: safeNum(els.days.value), temperatureC: safeNum(els.temp.value),
        breathlessness: safeNum(els.breath.value), chestTightness: safeNum(els.tight.value),
      ...
      files: {
        hasRecording: !!latestBlob,
        hasPhoto: !!(els.photo && els.photo.files && els.photo.files[0])
      }
    };
  }
